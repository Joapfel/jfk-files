SECRET

DATE 29 June 1963

PREPARATION OF OUTGOING SW MESSAGES

TO: Carl Prettin x-7824 SAS/BOB/DE
(Field Units Use Pseudo)

FROM: TSO/SNB (Div. or Branch)
(Field Units Use Pseudo)

Transmitted herewith is an Outgoing SW Message prepared for:

Rene GUTTAR ( Developmental Case )
(Cryptogram of Agent)

Message No.: 1

This message is located:

The message is located on the side opposite the small pencil "X" marks in the center of the page. (X on page one and II on page two)

It is suggested that the cover letter be placed:

The cover letter should be placed on the side with the pencil " X" marks.

NOTE: PLEASE ERASE THE PENCIL MARKS WITH A SOFT PENCIL ERASER BEFORE MAILING.

O. W. Hertson, Jr.
(Field Units sign in Pseudo)

Form 60-1 (OUT) SECRET

<table>
   <tr>
    <td>Page-footer</td>
   </tr>
   <tr>
    <td>Made by</td>
   </tr>
   <tr>
    <td>Lisa Mc</td>
   </tr>
   <tr>
    <td>8-1/2 x 11</td>
   </tr>
   <tr>
    <td>Datum:</td>
   </tr>
   <tr>
    <td>Pole to the left.</td>
   </tr>
  </table>
