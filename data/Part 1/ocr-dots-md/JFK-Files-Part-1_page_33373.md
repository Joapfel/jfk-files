OUTGOING MESSAGE

OMF 0 0 0 0 0 0

CLASSIFICATION:

SECRET

MESSAGE HANDLING INDICATOR

DATE TIME ORIG.

STAFF

CONF.:

INFO: FILE

DIRECTOR

MESSAGE REFERENCE NUMBER

0 DATE: DISSENT BY:
0 TIME: PER
0 REFS TO:
0 ....... #

TERRORIST "REFUGEES" IN EUROPE, AND COMMUNIST PARTIES HAVE BEEN ENGAGED IN CRITICALLY ANALYZING EVENTS LEADING TO CHILE COUP AND REASSESSING THEIR OWN PROSPECTS FOR SUCCESSFULLY DEALING WITH NON-COMMUNIST PARTIES AND ASSUMING POWER BY NON-VIOLENT MEANS. MIGHT ALSO MAKE POINT THAT WITH VIETNAM NO LONGER AN ISSUE, CHILE REPRESENTS ONE FOCUS FOR ENGENDERING UNITY BETWEEN COMMUNIST AND NON-COMMUNIST LEFT IN WESTERN EUROPE.

5. FOR INFORMATION OR POSSIBLE INTERVIEW ON EXTREME LEFTIST VIEWS/ACTIVITY, SUGGEST CAQUARRY OR ZDF REPORTER MIGHT CONTACT "SOCIALIST BUREAU", SPONSOR OF SOLIDARITY MEETING HELD FRANKFURT (27/79) (25701). FOR INFORMATION OR POSSIBLE INTERVIEW ON JUNTA VIEWS, SUGGEST CONTACT DEUTSCH-CHILEANISCHER BUND (SEE BERLIN 81306, OCT 73). (276994)

6. FILE: 200-126-107/11. E2 IMPDET.A

E/CAG

DATE: 30 MAY 1974
ORIG: CONNIE MOORE:SRM
UNIT: E/CAG
EXT: 66408/1289
C/EUR

C/E/CAG

REleasing OFFICeR
CLASSIFIcATION

COORDINATING OFFICERS

AUTHORIZING OFFICER

REPRODUCTION BY OTHER THAN THE ISSUING OFFICE IS PROHIBITED

E 2 IMPDET

CL BY: 004244

SECRET

SECRET

<table><tr><td>0</td></tr></table>

<table><tr><td>1</td></tr></table>

<table><tr><td>2</td></tr></table>

<table><tr><td>3</td></tr></table>

<table><tr><td>4</td></tr></table>

<table><tr><td>5</td></tr></table>

<table><tr><td>6</td></tr></table>

<table><tr><td>7</td></tr></table>

<table><tr><td>8</td></tr></table>

<table><tr><td>9</td></tr></table>

<table><tr><td>0</td></tr></table>

<table><tr><td>1</td></tr></table>

<table><tr><td>2</td></tr></table>

<table><tr><td>3</td></tr></table>

<table><tr><td>4</td></tr></table>

<table><tr><td>5</td></tr></table>

<table><tr><td>6</td></tr></table>

<table><tr><td>7</td></tr></table>

<table><tr><td>8</td></tr></table>

<table><tr><td>9</td></tr></table>

<table><tr><td>0</td></tr></table>

<table><tr><td>1</td></tr></table>

<table><tr><td>2</td></tr></table>

<table><tr><td>3</td></tr></table>

<table><tr><td>4</td></tr></table>

<table><tr><td>5</td></tr></table>

<table><tr><td>6</td></tr></table>

<table><tr><td>7</td></tr></table>

<table><tr><td>8</td></tr></table>

<table><tr><td>9</td></tr></table>

<table><tr><td>0</td></tr></table>

<table><tr><td>1</td></tr></table>

<table><tr><td>2</td></tr></table>

<table><tr><td>3</td></tr></table>

<table><tr><td>4</td></tr></table>

<table><tr><td>5</td></tr></table>

<table><tr><td>6</td></tr></table>

<table><tr><td>7</td></tr></table>

<table><tr><td>8</td></tr></table>
