<table>
    <tbody>
        <tr>
            <td rowspan="2">
                <table>
                    <tr><th>DISPATCH</th></tr>
                    <tr><td>OSHA/1<br/>SECRET</td></tr>
                </table>
            </td>
            <td colspan="3">PROCESSING ACTION</td>
        </tr>
        <tr>
            <td colspan="3">
                <table>
                    <tr><td>MAILED FOR SHIPPING</td></tr>
                    <tr><td>NO URCENTLY REQUIRED</td></tr>
                    <tr><td>ONLY QUALIFIED OFFICE CAN ADOLETRIURING</td></tr>
                    <tr><td>MICROPOLIS</td></tr>
                </table>
            </td>
        </tr>
        <tr>
            <td colspan="4">ACTION REQUIRED - REFERENCES</td>
        </tr>
        <tr>
            <td colspan="4">
                On 29 October QUTIMID reported a conversation between Guillermo RUIZ Perez, the Cuban Minister to Spain, and Manuel TELLO Troncoso, identified in the Diplomatic List as Counsellor of Embassy, regarding the postponement until 31 October of a meeting they were to have on 29 October. We have no information on the purpose of the meeting. We note that TELLO arrived in Spain in January 1969.
            </td>
        </tr>
        <tr>
            <td colspan="4">Howard R. SACODER</td>
        </tr>
        <tr>
            <td colspan="4">Distribution:</td>
        </tr>
        <tr>
            <td colspan="4">
                2 - COS, Santiago - To c/c T Pock<br/>
                2 - C/WHD<br/>
                2 - C/EUR<br/>
                2 - COS/WN, Miami - To c/c T Pock
            </td>
        </tr>
        <tr>
            <td colspan="4">68 COPY</td>
        </tr>
        <tr>
            <td>CROSS REFERENCE TO</td>
            <td>COMPANY SYMBOL AND NUMBER</td>
            <td>DATE</td>
            <td></td>
        </tr>
        <tr>
            <td></td>
            <td>OSHA 25624</td>
            <td>7 November 1969</td>
            <td></td>
        </tr>
        <tr>
            <td></td>
            <td>CLASSIFICATION</td>
            <td>NON-PIECE NUMBER</td>
            <td></td>
        </tr>
        <tr>
            <td></td>
            <td>SECRET</td>
            <td>201-735296</td>
            <td></td>
        </tr>
    </tbody>
</table>
